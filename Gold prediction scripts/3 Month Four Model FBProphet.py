...
df = df.filter(f"time > '{three_months_ago}' and time < '{today}'").select("time", "open")
...
minutes = 600
df = spark.sql("select * from forexcom_xauusd_3_months")
df = df.filter(f"time > '{three_months_ago}' and time < '{today}'").select("time", "high")
...
minutes = 600
df = spark.sql("select * from forexcom_xauusd_3_months")
df = df.filter(f"time > '{three_months_ago}' and time < '{today}'").select("time", "low")
...
minutes = 600
df = spark.sql("select * from forexcom_xauusd_3_months")
df = df.filter(f"time > '{three_months_ago}' and time < '{today}'").select("time", "open", "close", "high", "low")
df = df.withColumn("open", F.col("open").cast("double"))\
...
table_name = "3_month_model_daily_forecast"
